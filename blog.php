...
$stmt = $con->prepare('SELECT * FROM blogtable');
$stmt->execute();
$blogtb = $stmt->get_result();
?>
    <!----------------------------MAIN SECTION--------------------------->
    ...
    <?php 
    foreach($blogtb as $blogtable){?> 
        <section class="card">
            <div class="card-image">
                <img src="assets/img/<?php echo $blogtable['blog_image'];?>" alt="pic">
            </div>
           
            <div class="card-contents">
                <h6 class="card-title px-2">
                    <?php echo $blogtable['blog_title'];?>
                </h6>
                <i class="d-block mx-5"><i class="fa-solid fa-pen"></i> <?php echo $blogtable['blog_writer'];?></i>
                <i class="d-block mx-5"><i class="fa-solid fa-clock"></i> <?php echo $blogtable['blog_date'];?></i>
                ...
                <p class="text mx-3">
                <?php echo $blogtable['blog_content'];?>
                </p>
                <button class="see-more-btn  btn-sm text-bold btn-primary">Read more</button>
            </div>
        </section>
